...
# Constructing this value prevents triple backticks from being damaged
# when the code is copied from a chat interface.
CODE_FENCE = "\x60" * 3
...
def get_system_instructions():
    ...
    service_architecture = load_file(
        "context/service_architecture.md"
    )
...
def strip_code_fences(text):
    ...
    if cleaned.startswith(CODE_FENCE):
        ...
        if lines and lines[0].startswith(CODE_FENCE):
            lines = lines[1:]
...
def write_failed_response(
    ...
    output_path = (
        FAILED_RESPONSE_DIR
        / f"row_{row_idx}_{safe_name}.txt"
    )
    ...
    with open(
        ...
        file.write(f"Lead: {full_name}\n")
        file.write(f"Error: {error_message}\n")
        file.write("\n--- RAW RESPONSE ---\n")
        file.write(response_text or "")

    print(
        f"   Saved raw response to {output_path}"
    )


def get_parsed_agent_output(
    lead_text,
    row_idx,
    full_name,
):
    ...
    try:
        ...
        print(
            f"   Retrying row {row_idx} "
            "with stricter JSON instruction..."
        )
...
def get_row_crm_status(row):
    value = row.get(CRM_STATUS_COLUMN, "")
    return normalize_crm_status(str(value))
...
def should_process_row(row):
    ...
    agent_score = str(
        row.get(AGENT_SCORE_COLUMN, "")
    ).strip()

    followup = str(
        row.get(FOLLOWUP_DRAFT_COLUMN, "")
    ).strip()
...
def build_lead_text(row):
    return (
        ...
        f"{row.get('Full Name', '')}\n"
        f"Business Name: "
        f"{row.get('Business Name', '')}\n"
        f"Email: "
        f"{row.get('Email', '')}\n"
        f"Website URL: "
        f"{row.get('Website URL', '')}\n"
        f"Primary Structural Concern: "
        f"{row.get('Primary Structural Concern', '')}\n"
        f"Lead Source: "
        f"{row.get('Lead Source', '')}\n"
    )


def parse_agent_output(parsed):
    agent_score = str(
        parsed.get("Agent Score", "")
    ).strip()
    ...
    if (
        crm_status_new
        not in AGENT_ALLOWED_FINAL_STATUSES
    ):
        raise ValueError(
            "Invalid CRM status from model: "
            f"'{crm_status_new}'. Must be one of: "
            f"{sorted(AGENT_ALLOWED_FINAL_STATUSES)}"
        )
...
def process_sheet():
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        crm_status_current = get_row_crm_status(
            row
        )

        if (
            WRITE_NEW_STATUS_IF_BLANK
            and crm_status_current == ""
        ):
            worksheet.update(
                range_name=f"K{idx}",
                values=[["New"]],
        ...
        print(
            f"-> Analyzing Row {idx}: {full_name}"
        )

        try:
            parsed, _raw_response = (
                get_parsed_agent_output(
                    lead_text,
                    idx,
                    full_name,
                )
            ...
            if (
                ...
                print(
                    "   Skipped update: row moved to "
                    f"'{current_status_cell}' "
                    "during run."
                )
                continue
            ...
            print(
                f"   Success: updated row {idx}"
            )

        except Exception as error:
            print(
                f"   Error on row {idx}: {error}"
            )

    print(
        f"Done. Processed {processed_count} leads."
    )
